...
class PaintWindow(wx.Window):
		...
		def DrawLines(self, dc):
			...
			self.current_axis = [x_per_step, 0, xmin, xmax, ymin, ymax, wwidth, wheight]
...
def ddline(dc, linedata, defaultAxis):
	(xmin_ignore,xmax_ignore,ymin,ymax) = get4m(linedata)
	
	if ymin == ymax: return
	
	tempAxis = defaultAxis
	tempAxis[4], tempAxis[5] = ymin, ymax
	tempAxis[1] = 0
	px1, py1 = linedata[1]
	px1, py1 = zoom_out_axis(px1, py1, tempAxis)
	
	for px, py in linedata[1:]:
		px, py = zoom_out_axis(px, py, tempAxis)
		dc.DrawLine(px, py, px1, py1)
		px1, py1 =  px, py
...
def show_graph(linedata, forceaxis = 0, graph = 0, xzoom = 1):
	...
	pframe.xzoom = xzoom
	pframe.current_axis = [1 for i in range(0,8)] 
	# Due to wx, MouseMove event called in beginning
	# patch problem by init var value_set
